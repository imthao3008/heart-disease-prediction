...
with tab1:
    ...
    if predict_btn:
        ...
        with col_gauge:
            ...
            ax_g.fill_between(theta[:100], 0.7, 1.0, color='#2ecc71', alpha=0.3)
            ax_g.fill_between(theta[100:], 0.7, 1.0, color='#e05555', alpha=0.3)
        ...
        with col_info:
            ...
            if risk_factors:
                st.markdown("**🔴 Yếu tố nguy cơ phát hiện:**")
                for rf in risk_factors:
                    st.markdown(f"<span style='color:#e05555; font-size:13px'>{rf}</span>",
                                unsafe_allow_html=True)
